...
public abstract class AbstractSendMessageProcessor extends AsyncNettyRequestProcessor implements NettyRequestProcessor {
    ...
    protected RemotingCommand msgCheck(final ChannelHandlerContext ctx,
        ...
        // broker权限是否可写，topic是否有序
        if (!PermName.isWriteable(brokerController.getBrokerConfig().getBrokerPermission())
                && brokerController.getTopicConfigManager().isOrderTopic(requestHeader.getTopic())) {
        ...
        // 获取topic配置
        TopicConfig topicConfig =
                brokerController.getTopicConfigManager().selectTopicConfig(requestHeader.getTopic());
        if (null == topicConfig) {
            int topicSysFlag = 0;
            ...
            topicConfig = brokerController.getTopicConfigManager().createTopicInSendMessageMethod(
