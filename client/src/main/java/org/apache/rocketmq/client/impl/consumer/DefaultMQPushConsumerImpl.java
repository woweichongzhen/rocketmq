...
public class DefaultMQPushConsumerImpl implements MQConsumerInner {
        ...
        if (!this.consumeOrderly) {
            ...
            if (processQueue.isLocked()) {
                if (!pullRequest.isLockedFirst()) {
                    ...
                    log.info("the first time to pull message, so fix offset from broker. pullRequest: {} NewOffset: " +
                                    "{} brokerBusy: {}",
                            pullRequest, offset, brokerBusy);
                    if (brokerBusy) {
                        log.info("[NOTIFYME]the first time to pull message, but pull request offset larger than " +
                                        "broker consume offset. pullRequest: {} NewOffset: {}",
                                pullRequest, offset);
                    }
     ...
     * 启动消费者
     *
     * @throws MQClientException 客户端启动异常
     */
        ...
        switch (serviceState) {
            case CREATE_JUST:
                log.info("the consumer [{}] start beginning. messageModel={}, isUnitMode={}",
                        ...
                        defaultMQPushConsumer.getMessageModel(), defaultMQPushConsumer.isUnitMode());
                ...
                if (defaultMQPushConsumer.getMessageModel() == MessageModel.CLUSTERING) {
                    defaultMQPushConsumer.changeInstanceNameToPID();
                }

                // 创建客户端实例
                mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(
                        defaultMQPushConsumer,
                        rpcHook);

                // 设置重新负载均衡的策略，消息模型，消费组，发送请求API
                rebalanceImpl.setConsumerGroup(defaultMQPushConsumer.getConsumerGroup());
                rebalanceImpl.setMessageModel(defaultMQPushConsumer.getMessageModel());
                ...
                // 拉取消息API包装
                pullAPIWrapper = new PullAPIWrapper(
                        mQClientFactory,
                        defaultMQPushConsumer.getConsumerGroup(), isUnitMode());
                pullAPIWrapper.registerFilterMessageHook(filterMessageHookList);

                // 已消费的偏移量存储
                if (defaultMQPushConsumer.getOffsetStore() != null) {
                    offsetStore = defaultMQPushConsumer.getOffsetStore();
                } else {
                    switch (defaultMQPushConsumer.getMessageModel()) {
                        case BROADCASTING:
                            offsetStore = new LocalFileOffsetStore(mQClientFactory,
                                    defaultMQPushConsumer.getConsumerGroup());
                            break;
                        case CLUSTERING:
                            offsetStore = new RemoteBrokerOffsetStore(mQClientFactory,
                                    defaultMQPushConsumer.getConsumerGroup());
                            break;
                        default:
                            break;
                ...
                if (getMessageListenerInner() instanceof MessageListenerOrderly) {
                    ...
                    consumeMessageService =
                            new ConsumeMessageOrderlyService(this,
                                    (MessageListenerOrderly) getMessageListenerInner());
                ...
                boolean registerOK = mQClientFactory.registerConsumer(defaultMQPushConsumer.getConsumerGroup(),
                        this);
                if (!registerOK) {
                    serviceState = ServiceState.CREATE_JUST;
                    consumeMessageService.shutdown();
                    throw new MQClientException("The consumer group[" + defaultMQPushConsumer.getConsumerGroup()
                ...
                log.info("the consumer [{}] start OK.", defaultMQPushConsumer.getConsumerGroup());
    ...
    public ConsumerRunningInfo consumerRunningInfo() {
        ...
        for (SubscriptionData sd : subSet) {
            ConsumeStatus consumeStatus =
                    this.mQClientFactory.getConsumerStatsManager().consumeStatus(this.groupName(), sd.getTopic());
