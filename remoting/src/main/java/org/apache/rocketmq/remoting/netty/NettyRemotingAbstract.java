...
public abstract class NettyRemotingAbstract {
    ...
    /**
     * 信号量，用于限制单向发送请求的最大数量，保护系统内存
     * Semaphore to limit maximum number of on-going one-way requests, which protects system memory footprint.
     */
    protected final Semaphore semaphoreOneway;

    /**
     * 信号量，用于限制异步请求的最大数量，保护系统内存
     ...
     *
     * @param permitsOneway Number of permits for one-way requests. 单向发送请求的限制，公平信号量
     * @param permitsAsync  Number of permits for asynchronous requests. 异步发送请求的限制，公平信号量
     */
     ...
     *
     * @param ctx channel handler context. 通道处理上下文
     * @param cmd request command.   请求命令
     */
    public void processRequestCommand(final ChannelHandlerContext ctx, final RemotingCommand cmd) {
        // 获取请求码的处理器和线程池
        final Pair<NettyRequestProcessor, ExecutorService> matched = processorTable.get(cmd.getCode());
        // 如果没有匹配的，使用默认的请求处理器
        final Pair<NettyRequestProcessor, ExecutorService> pair = null == matched ? defaultRequestProcessor :
                matched;
            ...
            Runnable run = new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        // 如果处理器为netty请求异步处理器，执行异步请求；否则同步处理请求，并做处理后的RPC回调，并同步触发回调方法
    ...
    /**
     * 执行回调处理，如果回调线程池为空，直接使用当前线程
     */
    private void executeInvokeCallback(final ResponseFuture responseFuture) {
        boolean runInThisThread = false;
        // 获取回调线程池，执行回调任务，并执行信号量减一
        ...
        // 如果执行回调任务异常，或没有回调线程池，使用当前线程执行回调任务
        if (runInThisThread) {
            try {
                responseFuture.executeInvokeCallback();
            } catch (Throwable e) {
        ...
        try {
            ...
            if (null == responseCommand) {
                if (responseFuture.isSendRequestOK()) {
                    // 如果超时没有返回，但是发送成功，抛出超时异常
                    throw new RemotingTimeoutException(RemotingHelper.parseSocketAddressAddr(addr), timeoutMillis,
                            responseFuture.getCause());
                } else {
                    // 如果超时没有返回，但是发送失败，抛出发送请求异常
    ...
    public void invokeAsyncImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis,
        ...
        if (acquired) {
            ...
            if (timeoutMillis <= 0) {
                ...
                String info =
                        String.format("invokeAsyncImpl tryAcquire semaphore timeout, %dms, waiting thread nums: %d " +
